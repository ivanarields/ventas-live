...
async function ensureDailyPedidoFromPayment(input: {
  ...
  if (current) {
    ...
    const isLivePedido = String(current.source ?? '').toLowerCase() === 'live_sales';
    if (isLivePedido) {
      // El flujo Live gestiona su propio total_amount via syncMainPedidoForLiveOrder.
      // MacroDroid no debe reescribir ese total al ingresar un pago fuera del rango Live.
      const { data, error } = await supabase
        .from('pedidos')
        .update({ customer_name: input.customerName, updated_at: new Date().toISOString() })
        .eq('id', current.id)
        .eq('user_id', INGEST_USER_ID)
        .select()
        .single();
      if (error) console.error('[daily pedido update live-safe]', error);
      return data ?? current;
    }
      ...
      if (pagoErr) {
        ...
        if (STORE_SUPABASE_URL && STORE_SERVICE_KEY && amount) {
          try {
            ...
            if (matched) {
              const serverUrl = Deno.env.get('SERVER_URL') ?? 'https://leidycandy.me';
