...
export function CartView({ items, onBack, onCheckout, onRemove, darkMode }: Props) {
  const total = cartTotal(items);
  const isEmpty = items.length === 0;

  return (
    <div className="flex flex-col h-[100dvh] overflow-y-auto" style={{ background: darkMode ? '#1a0c12' : '#fef1f5' }}>
      <header
        className="sticky top-0 z-20 backdrop-blur-md border-b px-5 h-14 flex items-center gap-3"
        style={{
          background: darkMode ? 'rgba(26,12,18,0.90)' : 'rgba(254,241,245,0.78)',
          borderColor: 'rgba(255,45,120,0.10)',
        }}
      >
        <button type="button" onClick={onBack} className="relative z-30 w-10 h-10 rounded-full flex items-center justify-center transition-colors" aria-label="Volver">
      ...
      <div className="flex-1 px-5 py-4 pb-32">
        {isEmpty ? (
          <div className="min-h-[70vh] flex flex-col items-center justify-center text-center" style={{ background: darkMode ? '#1a0c12' : '#fef1f5' }}>
          ...
          <div className="space-y-2.5">
            {items.map(item => (
              <div
                ...
                className="rounded-[22px] backdrop-blur border shadow-sm p-2.5 flex gap-3"
                style={{
              ...
              >
                ...
                <div className="min-w-0 flex-1 py-0.5">
                  ...
                  {item.size && <p className="mt-1.5 text-[10px] font-black uppercase tracking-widest" style={{ color: darkMode ? '#e8d0da' : '#9ca3af' }}>{item.size}</p>}
                </div>
                <button
                  ...
                  style={{ background: darkMode ? 'rgba(255,45,120,0.15)' : '#fff0f5' }}
                  aria-label="Eliminar producto"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ff2d78" strokeWidth="2.5"><path d="M18 6 6 18M6 6l12 12"/></svg>
                </button>
              </div>
            ))}
          </div>
      ...
      {!isEmpty && (
        <footer
          className="fixed bottom-2 left-3 right-3 max-w-[406px] mx-auto rounded-[24px] backdrop-blur-md border px-4 pt-3 pb-4 space-y-3 z-20 shadow-[0_14px_35px_rgba(0,0,0,0.1)]"
          style={{
            background: darkMode ? 'rgba(26,12,18,0.95)' : 'rgba(254,241,245,0.95)',
            borderColor: darkMode ? 'rgba(255,255,255,0.10)' : 'rgba(255,45,120,0.10)',
          }}
        >
