...
function buildPhotosPath({ phone, orderDate, mainPedidoId, days = 4, receiptOnly = false }: PhotosRequest) {
  ...
  if (receiptOnly) params.set('mode', 'comprobantes');
...
export function OrderChatPhotoSelector({
  ...
  useEffect(() => {
    ...
    const photosRequest = fetchOrderChatPhotos({
      phone,
      orderDate,
      mainPedidoId,
      days,
      receiptOnly: showComprobantes,
    });
    const response = showComprobantes
      ? photosRequest.then((data) => ({ data, dayOrdersResp: null }))
      : Promise.all([
        photosRequest,
        apiFetch(`/api/live-sales/day-orders?phone=${encodeURIComponent(phone)}`).catch(() => null),
      ]).then(([data, dayOrdersResp]) => ({ data, dayOrdersResp }));
  ...
  }, [phone, orderDate, days, mainPedidoId, showComprobantes]);
  ...
  const pendingComprobantes = comprobantes.filter(photo => {
    const status = String(photo.payment_status ?? '');
    const verified = verifiedIds.has(photo.id)
      || status === 'verificado_macrodroid'
      ...
      <div className="space-y-2">
        ...
        <div className="flex h-16 items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-gray-100 bg-gray-50">
          ...
          <span className="text-[11px] text-gray-400">
            {showComprobantes ? 'Cargando comprobante...' : 'Buscando fotos y selección IA...'}
          </span>
        </div>
      </div>
    );
